...
function timeLabel(value:string|null,fallback:string){
  return value?new Date(value).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit',hourCycle:'h23',timeZone:'UTC'}):fallback;
}

function midpointTimeLabel(start:string|null,end:string|null){
  if(!start||!end)return '12:00';
  return timeLabel(new Date((new Date(start).getTime()+new Date(end).getTime())/2).toISOString(),'12:00');
}
...
export default function Dashboard(){
  ...
  const loadDashboard=useCallback(async()=>{setLoading(true);try{const [incidentItems,dashboard]=await Promise.all([api<Incident[]>('/incidents'),api<DashboardMetrics>('/dashboard')]);setIncidents(incidentItems);setMetrics(dashboard)}catch{location.href='/login'}finally{setLoading(false)}},[]);
  ...
  return <AppShell title="Operational overview" actions={<><AppModeToggle onModeChange={loadDashboard}/><a className="app-cta" href="/incidents/new"><Plus size={17}/>New analysis</a></>}>
    ...
    <section className="command-grid">
      <article className="command-card trend-card"><div className="card-title"><div><span>LOG EVENT VOLUME</span><h2>Events across the latest calendar day</h2></div><b>{metrics?.event_volume.total?`${metrics.event_volume.total} EVENTS`:'NO TIMED EVENTS'}</b></div><figure className="event-chart" aria-label={`Log events by hour. Y axis ranges from 0 to ${maxEvents} events. X axis runs from midnight to midnight the next day in UTC.`}><span className="y-axis-title">Events</span><div className="y-axis-scale" aria-hidden="true"><span>{maxEvents}</span><span>{Math.round(maxEvents/2)}</span><span>0</span></div><div className="chart-area"><div className="chart-grid" aria-hidden="true"><i/><i/><i/></div><div className="large-chart">{eventBuckets.map((value,index)=><i key={index} title={`${value} event${value===1?'':'s'}`} style={{height:value?`${Math.max(value/maxEvents*100,4)}%`:'2px'}} className={value===maxEvents&&value>0?'hot':''}/>)}</div></div><div className="x-axis-ticks"><span className="tick-midnight">{timeLabel(metrics?.event_volume.start??null,'00:00')}</span><span className="tick-six">06:00</span><span className="tick-noon">{midpointTimeLabel(metrics?.event_volume.start??null,metrics?.event_volume.end??null)}</span><span className="tick-six-pm">18:00</span><span className="tick-end">{timeLabel(metrics?.event_volume.end??null,'00:00')}</span></div><figcaption className="x-axis-title">Time (UTC, one-hour buckets)</figcaption></figure></article>
